...
const Issues = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const response = await fetch('http://127.0.0.1:3000/api/issuesBySummary?accountIdentifier=demo&orgIdentifier=default&projectIdentifier=test');
                if (!response.ok) {
    ...
    const columns: GridColDef<RowData>[] = [
        {
            ...
            flex: 1,
            renderCell: (params) => (
                <Link to={`/issues/${params.row.summary}`}>{params.value}</Link>
            ),
        },
